...
func (s *Snow3gFunc) lfsrInitialisationMode(F uint32) {
...
func (s *Snow3gFunc) lfsrKeystreamMode() {
	v := (s.lfsr[0] << 8) ^ mulAlpha(byte(s.lfsr[0]>>24)&0xff) ^ s.lfsr[2] ^ (s.lfsr[11] >> 8) ^
...
func (s *Snow3gFunc) clockFsm(s15, s5 uint32) uint32 {
...
func InitSnow3g(k, iv [4]uint32) *Snow3gFunc {
	s := &Snow3gFunc{}
	...
	for i := 0; i < 32; i++ {
		...
		s.lfsrInitialisationMode(F)
	}

	return s
}

func (s *Snow3gFunc) generateKeystream(n int, ks []uint32) {

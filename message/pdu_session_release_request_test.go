...
func TestPDUSessRelReqUnmarshalBinary(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
	...
	}{
		{
			name: "Case 1",
			input: []byte{
				0x2e, 0x01, 0xbf, 0xd1, 0x59, 0x24,
			},
			expected: &PDUSessRelReq{
				PDUSessId: 1,
				PTI:       191,
...
func TestPDUSessRelReqMarshalBinary(t *testing.T) {
	t.Parallel()

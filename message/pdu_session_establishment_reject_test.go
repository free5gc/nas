...
func TestPDUSessEstRejUnmarshalBinary(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
	...
	}{
		{
			name: "Case 1",
			input: []byte{
				0x2e, 0x05, 0x01, 0xc3, // EPD, PDUSessId, PTI, MsgType
				// ie.Cause5GSM V
				0x00,
				// BackoffTimerValue : &ie.GPRSTimer3         0x37   TLV,       3B, 9.11.2.5
				0x37, 0x01, 0x06,
				// AllowedSSCMode    : &ie.AllowedSSCMode     0xF0    TV,       1B, 9.11.4.5
				// ie.EAPMsg TLV-E
				0x78, 0x00, 0x04,
				0x01, 0x02, 0x03, 0x04,
...
func TestPDUSessEstRejMarshalBinary(t *testing.T) {
	t.Parallel()
